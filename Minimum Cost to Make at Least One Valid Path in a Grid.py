    ...
    def minCost(self, grid: List[List[int]]) -> int:        
        ...
        while queue:
            ...
            for _, (offset_x, offset_y) in DIRECTIONS.items():
                next_x = x + offset_x
                next_y = y + offset_y
                cost = 1 if need_modify_to_reach(x, y, next_x, next_y) else 0

                if is_valid_cell(next_x, next_y) and distances[x][y] + cost < distances[next_x][next_y]:
                    distances[next_x][next_y] = distances[x][y] + cost
                    if cost == 1:
